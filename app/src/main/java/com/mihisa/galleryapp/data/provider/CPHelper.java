...
import com.mihisa.galleryapp.data.filter.FoldersFileFilter;
...
/**
 * Created by insight on 30.03.18.
 */
...
public class CPHelper {
    ...
    private static String getHavingCause(int excludedCount) {
        if(excludedCount == 0)
            return "(";
    ...
    private static Observable<Album> getAlbums(Context context, ArrayList<String> excludedAlbums, SortingMode sortingMode, SortingOrder sortingOrder) {
        ...
        if(Prefs.showVideos()) {
            query.selection(String.format("%s=? or %s=?) group by (%s) %s ",
                    // TODO Why 2 Media_Type ?
                    MediaStore.Files.FileColumns.MEDIA_TYPE,
                    MediaStore.Files.FileColumns.MEDIA_TYPE,
                    MediaStore.Files.FileColumns.PARENT,
                    getHavingCause(excludedAlbums.size())));
    ...
    public static io.reactivex.Observable<Media> getMedia(Context context, Album album) {
    ...
    public static io.reactivex.Observable<Media> getMedia(Context context, Album album, SortingMode sortingMode, SortingOrder sortingOrder) {
