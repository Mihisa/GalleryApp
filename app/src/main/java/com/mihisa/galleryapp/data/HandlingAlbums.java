...
public class HandlingAlbums extends SQLiteOpenHelper {
    ...
    public void excludeAlbum(SQLiteDatabase db, Album album) {
        changeStatusAlbum(db, album, EXCLUDED);
    }

    public void addFolderToWhiteList(String path) {
        SQLiteDatabase db = getWritableDatabase();
        changeStatusAlbum(db,Album.withPath(path), INCLUDED);
        db.close();
    }
    ...
    private void changeStatusAlbum(SQLiteDatabase db, Album album, int status) {
    ...
    private static boolean exist(SQLiteDatabase db, String path) {
        Cursor cur = db.rawQuery(String.format("SELECT EXISTS(SELECT 1 FROM %s WHERE %s=? LIMIT 1)", TABLE_ALBUMS, ALBUM_PATH),
                new String[] {path});
    ...
    public void setPinned(String path, boolean pinned) {
        ContentValues values = new ContentValues();
    ...
    @NonNull public static AlbumSettings getSettings(SQLiteDatabase db, String path) {
        ...
        try {
            if(exist(db, path)) {
                cursor = db.query(
                        TABLE_ALBUMS, StringUtils.asArray(
                                ALBUM_COVER_PATH,
                                ALBUM_SORTING_MODE,
                                ALBUM_SORTING_ORDER,
                                ALBUM_PINNED),
                ...
                if(cursor.moveToFirst())
                    return new AlbumSettings(cursor.getString(0),
                            cursor.getInt(1),
                            cursor.getInt(2),
                            cursor.getInt(3));

                } else
                    db.insert(TABLE_ALBUMS, null, getDefaults(path));
